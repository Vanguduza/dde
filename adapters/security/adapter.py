"""Security T1 worker adapter (DDE-045, Chapter 8.1 / 9.2 / 9.3).

`start()` requires an active `capability.security` lease then runs
in-process SAST. The capability is PURE_READ — no Chapter 12.4 journal.
...
WORKER_ID = "worker.security-sast-v1"
CAPABILITY_SECURITY_ID = "capability.security"
DECLARED_CAPABILITIES = frozenset(
    {CAPABILITY_REPOSITORY, CAPABILITY_TESTING, CAPABILITY_SECURITY}
)
SECURITY_IDENTITY = ProfileIdentity(
    model_version="none",
    harness_version="security-sast-v1",
    toolset_manifest="in-process.sast",
    image_digest="local-security",
)

_NO_PREPARE = (
    "Security adapter refuses start() without prepare() and a bound "
    "security_mode — never a fabricated successful handle"
)


class SecurityWorkerAdapter:
    def __init__(
        self,
        workspaces: WorkspaceService | None = None,
        leases: CapabilityLeaseService | None = None,
        *,
        scanner: InProcessSecurityScanner | None = None,
    ) -> None:
        self._workspaces = workspaces
        self._leases = leases
        self._scanner = scanner or InProcessSecurityScanner()
    ...
    async def register(self) -> Registration:
        return Registration(
            worker_id=WORKER_ID,
            worker_profile_id=PROFILE_SECURITY,
            declared_capabilities=DECLARED_CAPABILITIES,
        )

    async def health(self) -> WorkerHealth:
        return WorkerHealth(healthy=True, detail="security adapter registered")
    ...
    async def prepare(
        ...
        if action is None or not action.security_mode:
            raise DdeError(
                "POLICY_DENIED",
                "SecurityWorkerAdapter requires a bound security_mode action",
    ...
    async def start(self, worker_run: WorkerRun) -> RunHandle:
        ...
        if not workspace.workspace_path:
            raise DdeError(
                "POLICY_DENIED",
                "security scan requires a workspace filesystem path",
        ...
        await self._leases.require_active(
            ...
            capability_id=CAPABILITY_SECURITY_ID,
        )
        result = await self._scanner.scan(
            SecurityScanSpec(root=workspace.workspace_path, mode=action.security_mode)
        )
        handle = RunHandle(
    ...
    async def request_pause(self, worker_run: WorkerRun) -> PauseResult:
        del worker_run
        return PauseResult(accepted=False, reason="security scan is synchronous")

    async def resume(
        self, worker_run: WorkerRun, checkpoint_ref: str | None
    ) -> ResumeResult:
        del worker_run, checkpoint_ref
        return ResumeResult(accepted=False, reason="security scan is synchronous")

    async def checkpoint(self, worker_run: WorkerRun) -> CheckpointRef:
        del worker_run
        return CheckpointRef(
            ref=None, accepted=False, reason="security scan is synchronous"
        )

    async def cancel(self, worker_run: WorkerRun, reason: str) -> CancelResult:
        del worker_run
        return CancelResult(accepted=False, reason=reason or "security scan")
    ...
    async def terminate(self, worker_run: WorkerRun) -> TerminationResult:
        del worker_run
        return TerminationResult(terminated=True, detail="security scan finished")

    async def cleanup(self, worker_run: WorkerRun) -> CleanupResult:
        self._handles.pop(worker_run.run_id, None)
        return CleanupResult(cleaned=True, detail="security scan handles cleared")
