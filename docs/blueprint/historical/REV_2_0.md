...
**Amended:** 22 August 2026 — safety-envelope mechanisms landed and specified in place (stop authority, attempt budgets, guardrail classification), design-gate toolchain adopted (EDR-0008), queue-closure policies recorded (usage metering, batch approval, routing health eviction, T2 termination scope, oracle confidence semantics). Chapter-local notes marked *(amended 2026-08-22)*; nothing renumbered, nothing superseded. **23 August 2026 — intentional-stop classification wired at its real site and the resume path brought under the same armed-stop law** (Ch.12.3 matrix row + Ch.12.4 new-mutation rule; EDR-0010 accepted, EDR-0012 wiring). **24 August 2026 — UI template/component sourcing brought under donor governance** (Ch.13.8 amendment from the anti-generic-output research; distinctiveness gates staged in `docs/planning/gap-closure-record.md §6.5` and playbook §10). **24 August 2026 (second sitting) — Frontend Studio workstream chartered beyond the original §18.3 chain**: `DDE-065..068` (generation-prompt compiler, donor discovery, studio surface, visual verification & critique loop) plus mobile targets planned as `DDE-069` — named in the §18.3 appended-workstreams block, Definition-of-Polished gate battery staged in Ch.11, conformance-by-construction codified in Ch.13.8; sources `docs/planning/product-studio-charter.md`, `docs/planning/frontend-studio-gui-spec.md`. The Orca-routing posture had already been amended into Ch.6.9 from `docs/planning/orca-routing-research-integration.md`. **24 August 2026 (third sitting) — sandbox/egress and context-layer research intake recorded** from `docs/planning/opensandbox-graft-research-integration.md`: OpenSandbox's egress-sidecar containment design noted as the donor pattern EDR-0011's decision memo must evaluate (Ch.7.2 note), and Graft's deterministic symbol-graph + cached index-time summaries noted as the evaluated alternative path to query-time embeddings before any semantic default-on (Ch.5.2 note); triggers staged in `docs/planning/gap-closure-record.md §6.8`.
...
> **Definition-of-Polished battery for generated-product screens** *(amended 2026-08-24; `docs/planning/product-studio-charter.md` §4, staged via DDE-068)*: a screen generated by the Frontend Studio workstream reaches a merged state only after passing the blocking battery — design lints DD201–DD207 clean, silhouette-distinct against the generic-layout corpus, believable-density floor, VLM rubric ≥ threshold OR an explicit human pixel sign-off through the approvals surface, honesty tests green, per-interaction motion spec present including reduced-motion degradation, and provenance traceable to the originating PRD. Until each gate's executor is wired, it renders as deferred, never implicitly handled; the sign-off vocabulary (`prototype_pixel_signoff`) is added through the ordinary contract path, not assumed to exist.
...
**Conformance-by-construction on authoring surfaces** *(amended 2026-08-24; `docs/planning/frontend-studio-gui-spec.md` §4.5)* — the authoring-side twin of the donor/lint governance above: any DDE authoring surface (GUI or generated prompt) offers only token-valid values drawn from the token sheet's generated modules, per target; freehand literal style input (raw color/font/spacing/duration/easing) is refused by law, so DD201–DD206 violation classes become unrepresentable at authoring time rather than caught at CI time.
...
Frontend Studio compile, donor-discovery and VLM-critique costs are in-budget-accounted — they ride this overhead budget (Ch.16.4), not beside it — with instrumentation landing through DDE-041's budget instrumentation *(amended 2026-08-24; `docs/planning/product-studio-charter.md`)*.
...
**Appended workstreams — chartered beyond the original §18.3 chain** *(amended 2026-08-24; numbered per the `docs/planning/mission-numbering-note.md` discipline; charters live in the named planning docs until folded into a future §18 revision)*:
`DDE-065` generation-prompt compiler — PRD → deterministic, provenance-pinned UI prompt; fail-closed on unresolved art-direction/token inputs (art-direction includes dials + DESIGN.md grammar per `docs/planning/design-tooling-integration.md`) ⟨`docs/planning/product-studio-charter.md`⟩ · `DDE-066` donor discovery + feature-function taxonomy — classified search fan-out grouped by PRD feature category ⟨Ch.13.8; EDR-0015 gate⟩ · `DDE-067` Frontend Studio surface — Gateway-client GUI over compilation/donor/authoring flows ⟨`docs/planning/frontend-studio-gui-spec.md`; conformance-by-construction law in Ch.13.8⟩ · `DDE-068` visual verification & critique loop — blocking Definition-of-Polished battery including first-party DD207+ / harvested tell+WIG lints ⟨Ch.11 amendment below; EDR-0016 gate; `design-tooling-integration.md`⟩ · `DDE-069` mobile profiles (provisional) — Expo/React Native, Capacitor, Jetpack Compose first-class; multi-target token SSOT via generator emitters ⟨GUI spec §12; behind its own artifact-generation EDR⟩.
...
> **Production call sites (DDE-064, 2026-08-27).** `ReadinessReview.run` is the Chapter 18.6 production caller: it evaluates every named candidate (`context_critic`, `route_critic`, `model_assisted_planning`, `simulation_model`, `retriever`) through `evaluate_candidate` and appends a durable `readiness.reviewed` audit event. Unmeasured counterfactuals fail-close to KEEP. `evaluate_candidate` never deletes a subsystem; `PROPOSE_EDR` is the only non-keep verdict and still does not delete. Route critic remains unimplemented (honest zeros at `ControlPlaneOverheadService.record_for_worker_run`); KEEP without a supplied counterfactual. S7 Chapter 18.2 exit rows and remaining Chapter 18.3 S7 landings are named in `engine.readiness.inventory.S7_PRIOR_LANDINGS` (not re-implemented here). This pass records no removal EDR.
