...
| R5 | **Silent drift** | `dde-studio.yml` currently typechecks and compiles but runs **no client tests**, and no metric tracks aesthetic debt; each small deviation passes unnoticed | Studio CI runs the suite (4.3), debt budget (4.14), scorecards (8) |
...
Every PR that mines a repo must carry a `References:` footer citing `repo@commit/path` per extracted item and the layer mined (per 2.1). A cumulative **provenance ledger** lives at `docs/design/provenance-ledger.md` *(path proposed by this playbook; create on first row)* — append-only, one row per extraction. Reviewers reject unclosed borrowings; the ui-review skill (6.3) checks the footer.
...
Skills are self-contained instruction sets stored under `.cursor/skills/<name>/SKILL.md` *(all proposed; authored only after owner review of this document)*. The binding rule file (4.1) tells agents when to load which. Pattern parity with the Farm OS skill set is intentional.
...
|---|---|---|---|---|
| 7.1 | `schemas/design/tokens.json` + `generate_contracts.py` extension *(proposed)* | Token SSOT → generated `tokens.ts`; drift detection. Adapts Style Dictionary/Tokens Studio SSOT+codegen+drift practice ([devcheolu](https://devcheolu.com/en/posts/0sa4JzPKHIoeWmwdx6Am), [Tokens Studio docs](https://docs.tokens.studio/fundamentals/design-tokens/), [sd-transforms](https://github.com/tokens-studio/sd-transforms)) without Figma or a new dependency | ci.yml generated-drift step ▣ | Build cost only |
| 7.2 | `node:test` string/DOM fingerprints (existing germ) | Structural + style regression per surface; copy law | `npm test` → dde-studio.yml ⚙ (4.3) | Built-in |
...
Adoption phasing: 7.1–7.3 + 7.7 are Phase-0 (before the next feature UI); 7.6 with them; 7.4–7.5 and 7.8 land behind one combined dependency-admission EDR (Phase 1).
...
| DDE-internal anchors | `AGENTS.md` · `.cursor/rules/mission-chapter-gate.mdc` · `interfaces/dde-studio/shared/ui/base.ts` (token block) · `shared/overviewVisual.test.ts` + `shared/clientHonesty.test.ts` (gate germs) · `.github/workflows/ci.yml` (drift step) + `dde-studio.yml` (gap) · `scripts/generate_contracts.py` · `docs/planning/gap-closure-record.md` · `interfaces/dde-studio/docs/overview-mockup-alignment.md` |

*End of DDE frontend & UX playbook — proposed for owner review.*
