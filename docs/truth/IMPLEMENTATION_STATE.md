...
| Production VEKL for target-application manufacturing | `IMPLEMENTED_PARTIAL` | Blueprint §26A / AD-048 remain canonical and DDE-075/076/077/080/081/082/083 remain the locked owners. The `0038` resource/stack/signature/activation/context/playbook foundation plus the `0039` Unit Knowledge Graph / deterministic GraphRAG / governed Target Project Truth Evolution tranche are implemented; the v2 tranche is CI-certified by core run `34497981504` and DDE Studio run `34491084691`, including reversible PostgreSQL migration traversal, **8/8** PostgreSQL integrations with explicit non-superuser same-tenant/different-project VEKL Unit isolation, **1576 passed / 7 skipped** Linux tests, **755 passed / 6 skipped / 563 deselected** Windows pure-unit tests, **223/223** contracts and **80/80** Knowledge structural browser tests. Stable Unit lineage/revisions, rebuildable typed graph projection, pinned determinism, hard-eligibility-before-ranking, immutable resolution/execution evidence, governed Truth Challenges, accepted-EDR supersession, affected-task `BLOCKED_ON_DECISION`, downstream invalidation and the real Knowledge workbench are therefore certified. The overall Production VEKL row remains `IMPLEMENTED_PARTIAL` only because broader DDE-082 adapter/authoring scope and DDE-083 adversarial/release certification remain separately owned. |
...
## 2B. Production VEKL Unit Knowledge Graph v2 — implemented and CI certified (2026-09-10)

The revised Unit Knowledge Graph / deterministic GraphRAG / Target Project Truth Evolution architecture is integrated as an additive extension of Blueprint §26A and AD-048. It does not create a second Project Truth, TaskGraph, Context, Governance, Verification, source-admission or executable-authority plane.

Implemented in the current branch:

- migration `0039` plus schema-generated tenant/project-scoped Unit Map, graph node/edge, retrieval-route, research-finding, conflict-observation, Truth Challenge/finding-link, graph-invalidation, immutable resolution-trace and append-only execution-knowledge-binding contracts;
- stable `unit_lineage_id` separated from `unit_revision_hash`, explicit `EXEMPT_BY_POLICY`, deterministic concern/routes and a pinned result-affecting GraphRAG envelope;
- graph-neighbourhood -> existing Source/VEKL hard eligibility -> lexical/semantic ranking -> deterministic contextual purpose/role selection;
- immutable `VEKLResolutionTrace` and append-only activation/context `VEKLExecutionKnowledgeBinding`, with freshness checks against truth/unit/stack/signature/graph/route/index/manifest state;
- provenance-bearing research/conflict/challenge flow where Project Truth conflict remains implementation-ineligible, while qualifying evidence may enter governed review;
- `project_truth_change` as an ordinary standing-forbidden Governance approval decided through existing human/project authorization, with truth writes only through `TruthService`, accepted EDR supersession and affected-task `BLOCKED_ON_DECISION`;
- deterministic Unit/graph/activation invalidation after accepted truth or contract drift;
- enabled Production Studio Knowledge workbench backed by live `vekl.projection`, with no fabricated active-resource or graph state;
- UI-bearing Unit product-quality metadata separating deterministic Frontend Contract/PXG + Screen Audit + silhouette gates from qualitative `visual_critique` and bounded `prototype_pixel_signoff`.

Local proof is 22/22 focused v2/Gateway tests, 753 passed / 6 skipped / 563 integration deselected across the earlier local pure-unit pass, 223/223 contract, 77/77 Studio host/shared client tests, strict MyPy across 596 source files, repository Ruff/format across 1,075 files, synchronized generated contracts/tokens/binding matrix, unchanged 70-finding design-lint ratchet, and clean Studio/desktop TypeScript plus production Vite build. Authoritative CI is now green: core run `34497981504` proves 1576 passed / 7 skipped Linux tests, 755 passed / 6 skipped / 563 deselected Windows pure-unit tests, 223/223 contract tests, 8/8 PostgreSQL integrations, including the explicit `dde_rls_probe` project-isolation regression, accepted Project Truth provisioning, generated-drift cleanliness, and reversible PostgreSQL migration traversal through `0038 <-> 0039`; DDE Studio run `34491084691` proves the Knowledge structural browser suite at 80/80 plus compile/design/visual accessibility jobs. The Unit Knowledge Graph v2 tranche is therefore implemented and CI-certified, while broader Production VEKL remains `IMPLEMENTED_PARTIAL` because DDE-082 adapter/authoring and DDE-083 adversarial/release scope remain separately owned. Evidence: `docs/evidence/vekl/VEKL_UNIT_KNOWLEDGE_GRAPH_V2_2026-09-10.md`.
