...
The OpenAI family is concretized as a pinned **discovery catalogue**, not an implicit install. The 2026-09-08 snapshot of `openai/plugins` at commit `1e285826e604f66f7208f7ac4dba0fe8341d1f57` records metadata for 10 high-value application-engineering plugin bundles and 98 direct Skill paths spanning iOS/macOS, web, data visualization, Android QA, product design, OpenAI/ChatGPT app development, security, data analytics and plugin/skill evaluation. The same pinned audit records bundle component-surface presence without granting authority: all 10 expose Skills and agent material, 7 expose scripts, 4 MCP configuration, 3 app configuration and 1 command surface. `vekl.openai.install_catalog_candidates` may persist only DDE-authored `PACKAGE_METADATA` / `S7_DISCOVERY_ONLY` candidates with no source URI, network/secret/filesystem scope or execution mode. Upstream Skill bodies, plugin components, apps, MCP servers, agents, scripts, tools, hooks or commands must still be resolved to exact publisher paths, admitted through Source Intelligence, licence-checked, decomposed and independently qualified before use. A social-media or personal Skill name such as `AppCreator` is never inferred to be OpenAI first-party merely because it ran in ChatGPT/Codex; it remains third-party/unresolved until provenance proves otherwise.

**Automation-corpus source law (EDR-0018 / AD-050).** Exact-pinned public automation corpus bytes are a distinct acquisition surface from EDR-0015 donor search. Acquisition requires the dedicated accepted host/path/method/quota/retention/revocation policy and existing CapabilityLease + ExternalEffect journaling. Production VEKL external-resource qualification uses a domain-neutral Source Intelligence base; DDE-069 design-source contracts remain frontend specializations and cannot be used to disguise automation artifacts as design-system admissions. Raw source and derived knowledge identities remain separate, and only sanitized separately hashed descriptors may become non-executable VEKL resources. Automation resources carry hard guidance polarity and a deterministic `WORKFLOW_AUTOMATION` concern route; anti-pattern material cannot satisfy positive guidance or execution roles.
