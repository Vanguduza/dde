...
from engine.chat.tables import (
...
class FrontendChatCheckpointService:
    ...
    async def create(
        ...
        *,
        ...
        conversation_context: dict[str, object] = {
            ...
            "context_domain": conversation.context_domain,
            ...
            "active_candidate_id": str(conversation.active_candidate_id)
            if conversation.active_candidate_id
            else None,
            "design_session_id": str(conversation.design_session_id)
            if conversation.design_session_id
            else None,
    ...
    async def restore_context(
        ...
        *,
        ...
        now = datetime.now(UTC)
        restored = checkpoint.conversation_context or {}
        raw_nodes = restored.get("selected_node_keys")
        selected_nodes = (
            [str(item) for item in raw_nodes] if isinstance(raw_nodes, list) else []
        )
        context_values: dict[str, object] = {
            "context_domain": restored.get("context_domain"),
            "active_artifact_ref": restored.get("active_artifact_ref"),
            "screen_key": restored.get("screen_key"),
            "selected_node_keys": selected_nodes,
            "viewport": str(restored.get("viewport") or "desktop-1440"),
        }
        for field in (
            "active_task_id",
            ...
            raw = restored.get(field)
            context_values[field] = UUID(str(raw)) if raw else None
        async with open_unit_of_work(
            self._engine, tenant_id=tenant_id, project_id=project_id
        ) as uow:
