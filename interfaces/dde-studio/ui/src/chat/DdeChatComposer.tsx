...
export interface DdeChatController {
...
export interface DdeChatComposerProps {
  ...
  readonly cursor: DdeChatController;
}

type DetailTab = "plan" | "activity" | "changes" | "checkpoints" | "context";

export function DdeChatComposer({
  thread,
  loading,
  error,
  busy,
  ...
  return (
    <section className="dde-chat dde-chat-cursor" data-testid="dde-chat" aria-label="DDE AI Chat">
      {expanded ? (
...
function ContextPanel({
  ...
  conversation: FrontendChatConversation | null;
  budget: FrontendChatContextBudget | null;
  refValue: string;
  onRefValueChange: (value: string) => void;
  onPin: () => Promise<void>;
  onUnpin: (ref: string) => Promise<void>;
}) {
  const allocation = budget?.allocation ?? null;
  const memory = budget?.memoryContext ?? [];
  const history = budget?.historyContext ?? [];
  const memoryTokens = memory.reduce((sum, item) => sum + item.estimatedTokens, 0);
  const historyTokens = history.reduce((sum, item) => {
    const value = item.estimatedTokens;
    return sum + (typeof value === "number" ? value : 0);
  }, 0);
  const allOmitted = Array.from(
    new Set([...(budget?.omittedRefs ?? []), ...(budget?.managedOmittedRefs ?? [])]),
  );
  return (
    <div data-testid="chat-context-panel" className="dde-chat-managed-context">
      <div className="dde-chat-context-budget">
        <strong>Managed context</strong>
        <span data-testid="chat-managed-token-budget">
          {allocation
            ? `${allocation.usedTokens.toLocaleString()} / ${allocation.budgetTokens.toLocaleString()} tokens · ${Math.round(allocation.utilization * 100)}%`
            : budget
              ? `${budget.estimatedTokens.toLocaleString()} / ${budget.budgetTokens.toLocaleString()} tokens`
              : "Not assembled"}
        </span>
      </div>
      {allocation ? (
        <div className="dde-chat-context-pins" data-testid="chat-token-allocation">
          <span className="dde-chat-chip">prompt {allocation.promptTokens.toLocaleString()}</span>
          <span className="dde-chat-chip">live DDE {allocation.liveContextTokens.toLocaleString()}</span>
          <span className="dde-chat-chip">memory {memoryTokens.toLocaleString()}</span>
          <span className="dde-chat-chip">history {historyTokens.toLocaleString()}</span>
          <span className="dde-chat-chip">system {allocation.systemReserveTokens.toLocaleString()}</span>
        </div>
      ) : null}
      {memory.length ? (
        <div className="dde-chat-context-pins" data-testid="chat-memory-recall">
          {memory.map((item) => (
            <span
              className="dde-chat-chip"
              key={item.memoryId}
              title={`${item.scopeKind} · ${item.trustClass} · ${item.storageBackend}${item.truncated ? " · excerpted" : ""}`}
            >
              memory {shortId(item.memoryId)} · {item.trustClass.toLowerCase()} · {item.estimatedTokens}t
            </span>
          ))}
        </div>
      ) : null}
      {budget?.historySummary ? (
        <div className="dde-muted" data-testid="chat-history-compaction">
          Compacted history: {budget.historySummary}
        </div>
      ) : null}
      {budget?.contextSnapshot ? (
        <div className="dde-muted" data-testid="chat-context-snapshot">
          Snapshot {shortId(budget.contextSnapshot.contextSnapshotId)} · {budget.contextSnapshot.reason}
          {budget.contextSnapshot.archiveStorageBackend
            ? ` · archive ${budget.contextSnapshot.archiveStorageBackend}`
            : ""}
        </div>
      ) : null}
      {allOmitted.length ? (
        <div className="dde-chat-refusal" data-testid="chat-context-omissions">
          Omitted under context policy: {allOmitted.join(", ")}
        </div>
      ) : null}
      <div className="dde-chat-search">
        <input
          aria-label="Pin context reference"
          value={refValue}
          onChange={(event) => onRefValueChange(event.target.value)}
          placeholder="@file:path, @screen:key or @memory:id"
        />
        <button type="button" onClick={() => void onPin()}>Pin</button>
      </div>
      <div className="dde-chat-context-pins">
        {conversation?.pinnedContextRefs.map((ref) => (
          <span className="dde-chat-chip" key={ref}>
            {ref}
            <button type="button" onClick={() => void onUnpin(ref)}>×</button>
          </span>
        ))}
      </div>
    </div>
