"""Chapter 6.5 table (DDE-035) -- routing_decision_outcomes.

Follows migration 0006's pattern exactly: replays the table's statements
...
_TABLES = ("routing_decision_outcomes",)
...
def upgrade() -> None:
    conn = op.get_bind()
    exists = conn.execute(
        text("SELECT to_regclass('public.routing_decision_outcomes')")
    ).scalar()
...
def downgrade() -> None:
    conn = op.get_bind()
    conn.execute(text("DROP TABLE IF EXISTS routing_decision_outcomes CASCADE"))
