"""DDE-069 Frontend Studio V2 tables -- contract, PXG and coverage.

Follows migration 0021's pattern: replays these tables' statements from
...
def upgrade() -> None:
    conn = op.get_bind()
    exists = conn.execute(text("SELECT to_regclass('public.pxg_nodes')")).scalar()
