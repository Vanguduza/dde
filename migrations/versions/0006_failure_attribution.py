"""Chapter 5.11 table (DDE-034) -- failure_attributions.

Follows migration 0005's pattern exactly: replays the table's statements
...
def upgrade() -> None:
    conn = op.get_bind()
    exists = conn.execute(
        text("SELECT to_regclass('public.failure_attributions')")
    ).scalar()
...
def downgrade() -> None:
    conn = op.get_bind()
    conn.execute(text("DROP TABLE IF EXISTS failure_attributions CASCADE"))
